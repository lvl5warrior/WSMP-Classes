 ...
 * path chosen yet), %wsmpclasses_path_tag% (a ready-to-use " [Fire]" style
 * bracketed, bold tag for name suffixes — resolves to a completely empty
 * string when no path is chosen, so it disappears rather than showing
 * empty brackets), and %wsmpclasses_class_level% (a combined
...
public final class ClassesPlaceholders extends PlaceholderExpansion {
    ...
    public String onRequest(OfflinePlayer player, @NotNull String params) {
        ...
        return switch (params.toLowerCase()) {
            ...
            case "path_tag" -> {
                if (data.chosenPath == null || data.chosenPath.isEmpty()) yield "";
                String pretty = Character.toUpperCase(data.chosenPath.charAt(0)) + data.chosenPath.substring(1);
                yield " §7[§b§l" + pretty + "§r§7]";
            }
