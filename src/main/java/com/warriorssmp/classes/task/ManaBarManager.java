...
public final class ManaBarManager {
    ...
    public void update(Player player, PlayerClassData data) {
        ...
        String cooldowns = switch (data.chosenClass) {
            case MAGE -> "§c🔥Fire " + readyText(data.fireballReadyAt) + " §7| §e⚡Lightning " + readyText(data.lightningReadyAt)
                    + " §7| §b❄Ice " + readyText(data.iceballReadyAt);
            case WARRIOR -> "§c🌀Whirlwind " + readyText(data.whirlwindReadyAt) + " §7| §6💨Charge " + readyText(data.chargeReadyAt);
