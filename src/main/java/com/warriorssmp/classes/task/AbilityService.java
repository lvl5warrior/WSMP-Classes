...
public final class AbilityService {
    ...
    public void tickPathEffects(Player player, PlayerClassData data) {
        if ("ice".equals(data.chosenPath) && data.investmentIn("mage_ice_perk") > 0) {
            player.addPotionEffect(new PotionEffect(PotionEffectType.WATER_BREATHING, 200, 0, false, false));
        } else if ("marksman".equals(data.chosenPath) && data.investmentIn("archer_marksman_perk") > 0) {
    ...
    private void startLavaWalkWatcher() {
        plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
            for (Player player : plugin.getServer().getOnlinePlayers()) {
                ...
                if (!"fire".equals(data.chosenPath) || data.investmentIn("mage_fire_perk") <= 0) continue;
    ...
    private void startTrapWatcher() {
        plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
            ...
            while (iterator.hasNext()) {
                ...
                // Owner-only visibility: Player#spawnParticle(...) called on
                // a specific Player instance only sends that particle
                // packet to THAT client — nobody else's game ever receives
                // it, so only the trap's owner sees this marker, and only
                // while they're actually near enough to matter.
                var owner = plugin.getServer().getPlayer(info.owner());
                if (owner != null) {
                    owner.spawnParticle(org.bukkit.Particle.HAPPY_VILLAGER,
                            trapLoc.clone().add(0.5, 0.6, 0.5), 2, 0.15, 0.1, 0.15, 0);
                }
    ...
    private void applyGuardianKnockbackImmunity(Player player, PlayerClassData data) {
        ...
        if ("guardian".equals(data.chosenPath) && data.investmentIn("warrior_guardian_perk") > 0) {
