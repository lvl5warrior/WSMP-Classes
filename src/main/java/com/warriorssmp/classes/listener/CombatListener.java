...
public final class CombatListener implements Listener {
    ...
    public void onClassCast(org.bukkit.event.player.PlayerInteractEvent event) {
        ...
        var castResult = switch (data.chosenClass) {
            ...
            case ARCHER -> {
                if (material != org.bukkit.Material.SPECTRAL_ARROW) yield null;
                event.setCancelled(true);
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        } else if (event.getDamager() instanceof org.bukkit.entity.Snowball snowball && snowball.getShooter() instanceof Player p) {
            attacker = p;
            PlayerClassData snowballData = plugin.dataStore().get(p.getUniqueId());
            if (snowballData.chosenClass == com.warriorssmp.classes.model.PlayerClass.MAGE) {
                plugin.abilityService().onIceballHit(p, snowballData, victim);
            }
